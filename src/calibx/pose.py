...
from .rendering import apply_json_qpos  # noqa: E402
from .visualization import draw_matches  # noqa: E402
...
def make_sam3_extractor(
    ...
    if not enabled:
        return None
    torch.set_float32_matmul_precision("highest")
    from .masking import Sam3Extractor

    return Sam3Extractor(bpe_path=None, ckpt_path=checkpoint_path)


def sam3_mask_to_numpy(
    mask, image_shape: tuple[int, int, int] | tuple[int, int]
) -> np.ndarray:
...
def load_camera_matrix(
    args: argparse.Namespace, image_shape: tuple[int, int, int]
) -> np.ndarray:
    height, width = image_shape[:2]
    ...
    return np.array(
        [
            [args.fallback_focal, 0.0, width / 2.0],
            [0.0, args.fallback_focal, height / 2.0],
            [0.0, 0.0, 1.0],
        ],
        dtype=np.float64,
    )
...
def fk_keypoints(model, data, keypoints: list[dict]) -> dict[str, np.ndarray]:
    ...
    for keypoint in keypoints:
        ...
        if body_id < 0:
            continue
        result[keypoint["name"]] = np.asarray(
...
def project_points(
    points_robot: np.ndarray, world_to_camera: np.ndarray, camera_matrix: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    hom = np.concatenate(
        [points_robot, np.ones((len(points_robot), 1), dtype=np.float64)], axis=1
    )
...
def keypoint_metrics(
    ...
    for keypoint in dream_keypoints(payload):
        ...
        pred_px, pred_cam = project_points(
            fk_points[name][None], pose["world_to_camera"], camera_matrix
        )
        gt_px = np.asarray(keypoint["projected_location"], dtype=np.float64)
        gt_cam = (
            np.asarray(
                keypoint.get("location", [np.nan, np.nan, np.nan]), dtype=np.float64
            )
            * gt_camera_scale
        )
        pixel_error = float(np.linalg.norm(pred_px[0] - gt_px))
        camera_error = (
            float(np.linalg.norm(pred_cam[0] - gt_cam))
    ...
    camera_errors = np.asarray(
        [row["camera_3d_error_m"] for row in rows], dtype=np.float64
    )
    ...
    summary = {
        ...
        "pck_5px": float((pixel_errors <= 5.0).mean() * 100.0)
        if len(pixel_errors)
        else float("nan"),
        "pck_10px": float((pixel_errors <= 10.0).mean() * 100.0)
        if len(pixel_errors)
        else float("nan"),
        "pck_20px": float((pixel_errors <= 20.0).mean() * 100.0)
        if len(pixel_errors)
        else float("nan"),
        "keypoint_add_mean_m": float(valid_camera_errors.mean())
        if len(valid_camera_errors)
        else float("nan"),
    }
    for threshold_mm in (10, 20, 40, 60):
        summary[f"keypoint_ADD<{threshold_mm}mm"] = (
            float((valid_camera_errors <= threshold_mm * 1e-3).mean() * 100.0)
            if len(valid_camera_errors)
            else float("nan")
        )
...
def load_joint_positions_from_payload(payload: dict) -> dict[str, float]:
    return {
        ...
        for joint in payload["sim_state"]["joints"]
    }


def render_orbit_views(
    model, data, args: argparse.Namespace, camera_matrix: np.ndarray, output_dir: Path
) -> list[Path]:
    import mujoco
    ...
    try:
        for view_index in range(args.views):
            ...
            camera_forward = np.mean(
                [np.asarray(scene_camera.forward) for scene_camera in scene_cameras],
                axis=0,
            )
...
def match_one_render(
    ...
    points0 = np.asarray(
        prediction.get("mkeypoints0_orig", np.empty((0, 2))), dtype=np.float32
    )
    points1 = np.asarray(
        prediction.get("mkeypoints1_orig", np.empty((0, 2))), dtype=np.float32
    )
    scores = np.asarray(
        prediction.get("mconf", np.ones(len(points0))), dtype=np.float32
    ).reshape(-1)
    ...
    world_points, valid_surface, geom_ids, body_ids = select_rendered_mesh_points(
        points1[selected], camera_path, model, data
    )
    ...
    if (
        max_pnp_correspondences is not None
        and len(image_points) > max_pnp_correspondences
    ):
    ...
    if len(image_points) < args.min_pnp_correspondences:
        view_record["pnp"] = {
            "status": "too_few_correspondences",
            "correspondences": int(len(image_points)),
        }
        return {
            **view_record,
            "_image_points": image_points,
            "_world_points": mesh_points,
            "_scores": mesh_scores,
        }

    try:
        pose = solve_camera_pose(
            image_points, mesh_points, mesh_scores, camera_matrix, args.pnp_threshold
        )
...
def save_pose_npz(
    output_dir: Path, best: dict, camera_matrix: np.ndarray, metrics: dict
) -> Path:
    pose = best["_pose"]
...
def process(args: argparse.Namespace) -> Path:
    ...
    observed_masked, input_mask = apply_input_mask(
        observed_rgb, args.mask_input, mask_extractor, args.mask_prompt
    )
    ...
    if input_mask is not None:
        Image.fromarray(input_mask.astype(np.uint8) * 255).save(
            run_dir / "input_mask.png"
        )

    model, data = make_model_and_data(
        args.mujoco_xml, args.width, args.height, camera_matrix, payload
    )
    ...
    if best["pnp"]["status"] != "success":
        summary = {
            ...
            "input_mask": {
                ...
                "mask_path": str(run_dir / "input_mask.png")
                if input_mask is not None
                else None,
            },
            "views": [
