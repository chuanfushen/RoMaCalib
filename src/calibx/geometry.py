...
def restrict_visual_geoms_to_bodies(
    ...
    """Hide visual geoms outside an exact body-name allowlist in place.

    The group mutation is deliberately made before rendering, visual bounds,
    and ``mjv_select``.  Those three stages therefore operate on precisely the
    same robot surface set rather than merely changing what the renderer sees.
    """
    if not body_names:
        return []
    ...
    if kept == 0:
        raise RuntimeError(
            f"None of the requested bodies has a geom in visual group "
            f"{visual_geom_group}: {names}"
        )
    return names
...
def subtree_visual_bounds(
    model, data, root_body_name: str
    ...
    if not chunks:
        raise RuntimeError(
            f"Body subtree {root_body_name!r} has no group-2 visual meshes"
        )
    vertices = np.concatenate(chunks)
    return vertices.min(axis=0), vertices.max(axis=0)
...
def render_frame_center_views(
    ...
    add_zed_camera(
        spec, "zed_render_camera", args.mesh_width, args.mesh_height, camera_matrix
    )
    model = spec.compile()
    ...
    if args.mesh_views == 1:
        ...
        azimuths = (
            azimuth_min
            + rng.uniform(0.0, azimuth_step)
            + np.arange(args.mesh_views) * azimuth_step
        )
...
def render_frame_views(
    ...
    for center_name in args.render_centers:
        render_paths.extend(
            render_frame_center_views(
                qpos,
                source_index,
                output_dir / center_name,
                args,
                camera_matrix,
                center_name,
            )
...
def select_rendered_mesh_points(
    points: np.ndarray, camera_path: Path, model, data
...
def _filter_matches_poselib(
    ...
    try:
        if geometry_type == "Homography":
            ...
            matrix, info = poselib.estimate_fundamental(
                points0, points1, ransac_options
            )
        else:
            raise NotImplementedError(geometry_type)
    except Exception:
...
def concatenate_visualizations(matching: np.ndarray, mesh: np.ndarray) -> np.ndarray:
    ...
    def pad(image: np.ndarray) -> np.ndarray:
        ...
        return cv2.copyMakeBorder(
            image, 0, target_height - image.shape[0], 0, 0, cv2.BORDER_CONSTANT
        )
...
def deduplicate_pnp_correspondences(
    ...
    for index in order:
        if any(
            np.sum((image_points[index] - image_points[other]) ** 2) < radius_squared
            for other in kept
        ):
            continue
...
def solve_camera_pose(
    ...
    image_points, world_points, scores = deduplicate_pnp_correspondences(
        image_points, world_points, scores
    )
    if len(image_points) < 6:
        raise RuntimeError(
            f"PnP needs at least 6 correspondences, found {len(image_points)}"
        )
    ...
    projected, _ = cv2.projectPoints(
        world_points.astype(np.float64), rvec, tvec, camera_matrix, None
    )
...
def render_pose_overlay(
    ...
    undistorted = cv2.undistort(
        rgb, raw_camera_matrix, distortion, None, undistorted_camera_matrix
    )
    spec = mujoco.MjSpec.from_file(str(args.mujoco_xml))
    ...
    model = spec.compile()
    camera_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_CAMERA, "zed_overlay_camera"
    )
    data = mujoco.MjData(model)
    joint_count = min(qpos.reshape(-1).shape[0], model.nq)
    data.qpos[:joint_count] = qpos.reshape(-1)[:joint_count]
    ...
    overlay[robot_mask] = (
        (1.0 - alpha) * overlay[robot_mask] + alpha * rendered[robot_mask]
    ).astype(np.uint8)
    for point in pose["image_points"][pose["inlier_indices"]]:
        cv2.circle(
            overlay, tuple(np.rint(point).astype(int)), 4, (0, 255, 0), -1, cv2.LINE_AA
        )
    return overlay
