...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-x", "--views", type=int, default=6, help="Number of orbit views to render.")
    ...
    parser.add_argument("--distance-scale", type=float, default=2.8, help="Camera distance as a multiple of arm radius.")
    parser.add_argument("--min-distance", type=float, default=1.2, help="Minimum camera distance.")
    parser.add_argument("--elevation", type=float, default=-20.0, help="Free-camera elevation in degrees.")
    parser.add_argument("--azimuth-offset", type=float, default=0.0, help="Azimuth offset in degrees.")
    parser.add_argument("--hide-collision", action="store_true", default=True, help="Render visual geoms only.")
    parser.add_argument(
        "--show-collision",
        dest="hide_collision",
        action="store_false",
...
def apply_json_qpos(model, data, json_positions: dict[str, float]) -> dict[str, float]:
    ...
    for json_name, position in json_positions.items():
        ...
        qpos_width = model.jnt_qposadr[joint_id + 1] - qpos_address if joint_id + 1 < model.njnt else model.nq - qpos_address
        if qpos_width != 1:
            raise ValueError(f"Joint {joint_name!r} has qpos width {qpos_width}; this script expects scalar joints.")
...
def visual_bounds(model, data, visual_geom_group: int = 2) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not points:
        points = [np.asarray(data.xpos[body_id], dtype=np.float64) for body_id in range(1, model.nbody)]
