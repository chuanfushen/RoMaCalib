...
def compute_orbit_camera(model, data, renderer, scene_option, center: np.ndarray, distance: float, azimuth: float, elevation: float):
    import mujoco
...
def render_frame(args: argparse.Namespace, model, data, renderer, scene_option, camera_id: int, camera_matrix: np.ndarray, index: int, json_path: Path) -> dict:
    import mujoco
...
def process(args: argparse.Namespace) -> Path:
    ...
    try:
        for ordinal, (index, json_path) in enumerate(frames, start=1):
            ...
            records.append(render_frame(args, model, data, renderer, scene_option, camera_id, camera_matrix, index, json_path))
    finally:
        renderer.close()
        ...
        "frames": [
            {
                ...
                "metadata": str(args.output_dir / f"{record['frame_index']:06d}" / "metadata.json"),
            }
            for record in records
        ],
    }
    manifest_name = "manifest.json" if args.num_shards == 1 else f"manifest_shard_{args.shard_index:02d}.json"
